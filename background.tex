...
This is obviously restricted to learning linear transformations, so to learn
non-linear functions, you first add a non-linear \textit{activation-function}:
...
...$. Using this we can then minimize the loss using gradient descent.
...
For classification
networks, a common function to have as the last activation function is the
softmax function shown in figure \ref{fig:softmax}. The softmax have the
